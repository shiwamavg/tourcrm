...
const moduleText = fnText + '\nglobalThis.result = { buildInvoicePdf };';
const sandbox = { require, console, Buffer, process, Date, path, globalThis: {} };
vm.createContext(sandbox);
vm.runInContext(moduleText, sandbox);
const buildInvoicePdf = sandbox.globalThis.result.buildInvoicePdf;
if (typeof buildInvoicePdf !== 'function') {
  console.error('buildInvoicePdf not found in sandbox');
  process.exit(1);
}
...
console.log('hasPDF', s.startsWith('%PDF-1.4'));
console.log('xref', s.indexOf('xref'));
console.log('stream', s.indexOf('stream\n'));
console.log('endstream', s.indexOf('endstream'));
console.log('BT', s.indexOf('BT '));
console.log('F1', s.indexOf('/F1'));
console.log('F2', s.indexOf('/F2'));
console.log('contentsRef', s.indexOf('/Contents 4 0 R'));
fs.writeFileSync(path.join(__dirname, 'tmp-generated-invoice.pdf'), buf);
console.log('wrote tmp-generated-invoice.pdf');
